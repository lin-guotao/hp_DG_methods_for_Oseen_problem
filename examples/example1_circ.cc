...
namespace example1_circ
{
  ...
  {
  public:
    OseenProblem(const MeshType mesh_type = MeshType::Mesh5,
  ...
  private:
    ...
    const std::string example_name = "example1_circ";
    ...
    if (std::filesystem::exists(cachefilename + "_triangulation.data"))
      {
        ...
        triangulation.set_manifold(0, SphericalManifold<2>(Point<2>(0.5, 1.)));
        triangulation.set_manifold(1, SphericalManifold<2>(Point<2>(0.5, 1.)));
      }
    else
      {
        ...
        for (const auto &cell : triangulation.active_cell_iterators())
          {
            if (mesh_type == MeshType::Mesh5 || mesh_type == MeshType::Mesh7)
              cell->set_material_id(0);
            else
              {
                if (cell->center().distance(Point<2>(0.5, 1.)) > radius)
                  cell->set_material_id(0);
                else
  ...
  OseenProblem<dim>::make_agglo_grid(const unsigned int mesh_level)
  {
    ...
    if (mesh_type == MeshType::Mesh5 || mesh_type == MeshType::Mesh6)
      {
        ...
        const unsigned int num_domain = (mesh_type == MeshType::Mesh5) ? 1 : 2;
        ...
        for (unsigned int i = 0; i < num_domain; ++i)
          {
            ...
            const unsigned int extraction_level_sub =
              std::min(mesh_level - (num_domain - 1) -
                         (int)(std::log(num_domain) /
                               std::log(max_elem_per_node)),
                       n_levels(tree));
    ...
    else if (mesh_type == MeshType::Mesh7 || mesh_type == MeshType::Mesh8)
      {
        std::cout << "     Partition with Metis." << std::endl;
        const unsigned int n_subdomains = 5 * (int)pow(4, mesh_level - 2);
      ...
      {
        AssertThrow(false,
                    ExcMessage("Only Mesh5 through Mesh8 are supported."));
      }
...
main()
{
  try
    {
      using namespace dealii;
      using example1_circ::FEType;
      using example1_circ::MeshType;

      for (const MeshType mesh_type :
           {MeshType::Mesh5, MeshType::Mesh6, MeshType::Mesh7, MeshType::Mesh8})
        {
          example1_circ::OseenProblem<2> oseen_problem(mesh_type,
                                                  FEType::P3P2,
                                                  /*Re =*/1.0);
          oseen_problem.run();
        }
